...
fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    PROPOSAL_MAP.with(|p &RefCell<StableBTreeMap,Proposal,VirtualMemory<std::rc::Rc<RefCell<Vec<u8>>>>>| p.borrow_mul().insert(key, value))
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        ...
        let res = p.borrow_mul().insert(key, value);
...
fn end_proposal(key: u64) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        ...
        let res = p.borrow_mul().insert(key, old_proposal);
